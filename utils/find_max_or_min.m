...
% function [values,location_indexes] = find_max_or_min(data, start_index, ...
%     location_window_start_and_end_indexes, max_or_min_value)
% Finds max or min value of data
% Example: data = DataPeaks_mean{1, 1}.data(:,1); [value, location] = find_max_or_min(data)
% if want to use for filtered data, use e.g.
% data_filtered = filter_data(data, fs, used_filter, filter_parameters,[],'no');
% [value_filtered_data, location_filtered_data] = find_max_or_min(data_filtered)
...
if nargin < 3 || isempty(location_window_start_and_end_indexes) ...
        || isnan(location_window_start_and_end_indexes)
...
if ~strcmp(max_or_min_value,'max') && ~strcmp(max_or_min_value,'min')
   disp('Incorrect max_or_min given. Set to max, i.e. finding the highest positive value')
   max_or_min_value = 'max';
end
...
% finding max or min value between given window
